...
@ExposesResourceFor(AdresseIntern.class)
@RequestMapping("/adresseInterns/search")
public class AdresseInternSearchController {

    @Autowired
    QueryService service;

    @Autowired
    AdresseInternRepository repository;
    ...
    public ResponseEntity<?> findFullTextFuzzy(PersistentEntityResourceAssembler assembler, @Param("q") String q) {
        ...
        String[] annotatedFields = Stream
                .of(AdresseIntern.class.getDeclaredFields())
                .filter(field -> field.isAnnotationPresent(org.hibernate.search.annotations.Field.class))
                .map(Field::getName)
                .toArray(size -> new String[size]);

        Stream<AdresseIntern> adresseInternStream;
        try {
            adresseInternStream = service.query(q, AdresseIntern.class, annotatedFields).stream();
        } catch (EmptyQueryException e) {
            adresseInternStream = StreamSupport.stream(repository.findAll().spliterator(), false);
        }

        final List<PersistentEntityResource> collect = adresseInternStream.map(assembler::toResource).collect(Collectors.toList());
