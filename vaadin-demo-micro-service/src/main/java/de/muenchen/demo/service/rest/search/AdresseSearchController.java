...
@ExposesResourceFor(Adresse.class)
@RequestMapping("/adresses/search")
public class AdresseSearchController {
    ...
    public ResponseEntity<?> findFullTextFuzzy(PersistentEntityResourceAssembler assembler, @Param("q") String q) {
        ...
        String[] annotatedFields = Stream
                .of(Adresse.class.getDeclaredFields())
                .filter(field -> field.isAnnotationPresent(org.hibernate.search.annotations.Field.class))
                .map(Field::getName)
                .toArray(size -> new String[size]);

        Stream<Adresse> adresseStream;
        try {
            adresseStream = service.query(q, Adresse.class, annotatedFields).stream();
