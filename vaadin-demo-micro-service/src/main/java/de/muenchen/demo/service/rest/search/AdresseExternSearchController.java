...
@ExposesResourceFor(AdresseExtern.class)
@RequestMapping("/adresseExterns/search")
public class AdresseExternSearchController {

    @Autowired
    QueryService service;

    @Autowired
    AdresseExternRepository repository;
    ...
    public ResponseEntity<?> findFullTextFuzzy(PersistentEntityResourceAssembler assembler, @Param("q") String q) {
        ...
        String[] annotatedFields = Stream
                .of(AdresseExtern.class.getDeclaredFields())
                .filter(field -> field.isAnnotationPresent(org.hibernate.search.annotations.Field.class))
                .map(Field::getName)
                .toArray(size -> new String[size]);

        Stream<AdresseExtern> adresseExternStream;
        try {
            adresseExternStream = service.query(q, AdresseExtern.class, annotatedFields).stream();
        } catch (EmptyQueryException e) {
            adresseExternStream = StreamSupport.stream(repository.findAll().spliterator(), false);
        }

        final List<PersistentEntityResource> collect = adresseExternStream.map(assembler::toResource).collect(Collectors.toList());
