...
public class WohnungTest {
    ...
    public void testCacheOnSave() {
        ...
        Adresse a1 = this.createAdresse("OIDC01");
    ...
    public void saveTest() {
        ...
        String oid = "OIDW0";
        Wohnung b1 = service.save(this.createWohnung(oid),this.createAdresse(oid));
    ...
    private Adresse createAdresse(String oid) {
        Adresse adresse = new Adresse();
        adresse.setOid(oid);
        adresse.setStrasse("Suedstr.");
        adresse.setHausnummer(20);
        adresse.setPlz(69584);
        adresse.setStadt("Frankfurt");
        return adresse;
    }
    ...
    public void updateTest() {
        ...
        Adresse adr = adresseService.read(b1.getAdresse().getOid());
        service.update(b1,adr);
    ...
    public void deleteTest() {
        ...
        Wohnung b1 = service.read(DomainConstants.M2_W018);
        assertNotNull(b1);
        service.delete(DomainConstants.M2_W018);
        Wohnung b2 = service.read(DomainConstants.M2_W018);
        assertNull(b2);
        System.out.println(String.format("Wohnung konnte mit OID '%s' aus der DB (und dem Cache) gelöscht werden.", DomainConstants.M2_W018));
    }
    ...
    public void copyListWohnungTest() {
        ...
        oids.add(DomainConstants.M2_W002);
        oids.add(DomainConstants.M2_W025);
        service.copy(oids);
        List<Wohnung> bs = service.query();
        assertEquals(x + 2, bs.size());
        System.out.println(String.format("Objekte mit der OID '%s' und der OID '%s' konnte erfolgreich in Objekt  kopiert (und in DB gespeichert) werden", DomainConstants.M2_B002, DomainConstants.M2_B025));
    ...
    public void WohnungDeleteListTest() {
        System.out.println("========== delete Wohnung Test ==========");
        ArrayList<String> oids = new ArrayList();
        oids.add(DomainConstants.M2_W039);
        oids.add(DomainConstants.M2_W029);
        service.delete(oids);
        Wohnung b1 = service.read(DomainConstants.M2_W039);
        assertNull(b1);
        Wohnung b2 = service.read(DomainConstants.M2_W029);
        assertNull(b2);
        System.out.println(String.format("Wohnung  mit OID '%s'und OID '%s' konnte aus der DB (und dem Cache) gelöscht werden.", DomainConstants.M2_W039, DomainConstants.M2_W029));
