...
public class Sachbearbeiter_ServiceImpl implements Sachbearbeiter_Service, Serializable {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(Sachbearbeiter_ServiceImpl.class);
    ...
    public Sachbearbeiter_ create(Sachbearbeiter_ sachbearbeiter) {
    	Sachbearbeiter_ returnSachbearbeiter;
        Future<Sachbearbeiter_> result = executor.submit(() -> client.create(sachbearbeiter));
        try {
            returnSachbearbeiter = result.get(TIMEOUT_VAL, TimeUnit.SECONDS);
        } catch (HttpClientErrorException e) {
            returnSachbearbeiter = Sachbearbeiter_FallbackDataGenerator.createSachbearbeiterFallback();
    ...
    public Sachbearbeiter_ update(Sachbearbeiter_ sachbearbeiter) {
    	Sachbearbeiter_ returnSachbearbeiter;
        Future<Sachbearbeiter_> result = executor.submit(() -> client.update(sachbearbeiter));
        try {
            returnSachbearbeiter = result.get(TIMEOUT_VAL, TimeUnit.SECONDS);
        } catch (HttpClientErrorException e) {
            returnSachbearbeiter = Sachbearbeiter_FallbackDataGenerator.createSachbearbeiterFallback();
    ...
    public List<Sachbearbeiter_> findAll() {
        List<Sachbearbeiter_> sachbearbeiters;
        Future<List<Sachbearbeiter_>> result = Executors.newCachedThreadPool().submit((Callable<List<Sachbearbeiter_>>) client::findAll);
        try {
            sachbearbeiters = result.get(TIMEOUT_VAL, TimeUnit.SECONDS);
            return sachbearbeiters;
        } catch (HttpClientErrorException e) {
            sachbearbeiters = Sachbearbeiter_FallbackDataGenerator.createSachbearbeitersFallback();
            LOG.error(e.getMessage());
    ...
    public List<Sachbearbeiter_> findAll(Link relation) {
        List<Sachbearbeiter_> sachbearbeiters;
        Future<List<Sachbearbeiter_>> result = executor.submit(() -> client.findAll(relation));
        try {
            sachbearbeiters = result.get(TIMEOUT_VAL, TimeUnit.SECONDS);
        } catch (HttpClientErrorException e) {
            sachbearbeiters = Sachbearbeiter_FallbackDataGenerator.createSachbearbeitersFallback();
    ...
    public Optional<Sachbearbeiter_> findOne(Link link) {
        Optional<Sachbearbeiter_> sachbearbeiter;
        Future<Optional<Sachbearbeiter_>> result = executor.submit(() -> client.findOne(link));
        try {
            sachbearbeiter = result.get(TIMEOUT_VAL, TimeUnit.SECONDS);
        } catch (HttpClientErrorException e) {
            sachbearbeiter = Sachbearbeiter_FallbackDataGenerator.createOptionalSachbearbeiterFallback();
            LOG.error(e.getMessage());
            showErrorNotification(I18nPaths.NotificationType.error, SimpleAction.read);
        } catch (TimeoutException e) {
            sachbearbeiter = Sachbearbeiter_FallbackDataGenerator.createOptionalSachbearbeiterFallback();
    ...
    public List<Sachbearbeiter_> querySachbearbeiter(String query) {
        List<Sachbearbeiter_> sachbearbeiters;
        Future<List<Sachbearbeiter_>> result = executor.submit(() -> client.findFullTextFuzzy(query));
        try {
            sachbearbeiters = result.get(TIMEOUT_VAL, TimeUnit.SECONDS);
        } catch (HttpClientErrorException e) {
            sachbearbeiters = Sachbearbeiter_FallbackDataGenerator.createSachbearbeitersFallback();
