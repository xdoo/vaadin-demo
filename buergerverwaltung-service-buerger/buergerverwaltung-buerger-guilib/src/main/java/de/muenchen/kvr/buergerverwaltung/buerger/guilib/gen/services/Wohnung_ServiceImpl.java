...
public class Wohnung_ServiceImpl implements Wohnung_Service, Serializable {
    ...
    public Wohnung_ create(Wohnung_ wohnung) {
        return client.create(wohnung);
    }
    public Wohnung_ defaultCreate(Wohnung_ bean) {
		showErrorNotification(I18nPaths.NotificationType.error, SimpleAction.create);
		return Wohnung_FallbackDataGenerator.createWohnungFallback();
	}
    ...
    public Wohnung_ update(Wohnung_ wohnung) {
        return client.update(wohnung);
    }
    public Wohnung_ defaultUpdate(Wohnung_ bean) {
    ...
    public List<Wohnung_> defaultFindAll() {
		showErrorNotification(I18nPaths.NotificationType.error, SimpleAction.read);
		return Wohnung_FallbackDataGenerator.createWohnungsFallback();
	}
    ...
    public List<Wohnung_> defaultFindAll(Link relation) {
		showErrorNotification(I18nPaths.NotificationType.error, SimpleAction.read);
		return Wohnung_FallbackDataGenerator.createWohnungsFallback();
	}
    ...
    public Optional<Wohnung_> defaultFindOne(Link link) {
		showErrorNotification(I18nPaths.NotificationType.error, SimpleAction.read);
		return Wohnung_FallbackDataGenerator.createOptionalWohnungFallback();
	}
	 ...
	 */
    @Override
    @HystrixCommand(fallbackMethod = "defaultQueryWohnung")
    public List<Wohnung_> queryWohnung(String query) {
        return client.findFullTextFuzzy(query);
    }
    public List<Wohnung_> defaultQueryWohnung(String query) {
		showErrorNotification(I18nPaths.NotificationType.error,SimpleAction.read);
		return Wohnung_FallbackDataGenerator.createWohnungsFallback();
	}
