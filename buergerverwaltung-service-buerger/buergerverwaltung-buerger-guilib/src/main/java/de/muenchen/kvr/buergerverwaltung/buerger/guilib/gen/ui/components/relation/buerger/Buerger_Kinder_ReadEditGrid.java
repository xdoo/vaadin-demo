 ...
 * @author claus
 */
public class Buerger_Kinder_ReadEditGrid extends CustomComponent {
	 ...
	 */
    public Buerger_Kinder_ReadEditGrid(Buerger_ViewController controller, String navigateToRead, String navigateToCreate, String navigateToAdd, String navigateToDelete) {

        this.controller = controller;

        grid = new GenericGrid<Buerger_>(controller.getModel().getSelectedBuergerKinder(), Buerger_.Field.getProperties());
		...
		if(navigateToDelete!=null){
	        ...
	        Buerger_AssociationListActions listAction = new Buerger_AssociationListActions(
	                () -> grid.getSelectedEntities().stream()
	                        .map(buerger -> new Association<>(buerger, Buerger_.Rel.kinder.name()))
